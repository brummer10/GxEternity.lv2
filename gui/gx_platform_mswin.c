...
void gx_gui_create_window_and_surface(gx_eternityUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_eternityUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_eternityUI *ui) {
	...
	UnregisterClass(TEXT("gx_eternityUIClass"), NULL);
}
...
void event_handler(gx_eternityUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_eternityUI *ui = (gx_eternityUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
