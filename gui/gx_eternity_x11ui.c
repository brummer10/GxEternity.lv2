...
static void resize_event(gx_eternityUI * const ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_eternityUI * const ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_eternityUI* ui = (gx_eternityUI*)malloc(sizeof(gx_eternityUI));
	...
	ui->controls[0] = (gx_controller) {{1.0, 1.0, 0.0, 1.0, 1.0}, {40, 55, 81, 81}, false,"POWER", BSWITCH, BYPASS};
	ui->controls[1] = (gx_controller) {{0.25, 0.25, 0.0, 1.0, 0.01}, {220, 55, 81, 81}, false,"DRIVE", KNOB, DRIVE};
	ui->controls[2] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {380, 55, 81, 81}, false,"GLASS", KNOB, GLASS};
	ui->controls[3] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {540, 55, 81, 81}, false,"LEVEL", KNOB, LEVEL};
	ui->block_event = -1;
	ui->start_value = 0.0;
	ui->sc = NULL;
	ui->set_sc = 0;
...
static void cleanup(LV2UI_Handle handle) {
	gx_eternityUI * const ui = (gx_eternityUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(const gx_eternityUI * const ui, const gx_controller * const knob) {
	...
	cairo_pattern_add_color_stop_rgba (pat, 0,  0.2, 0.2, 0.25, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 0.25,  0.15, 0.15, 0.15, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 0.5,  0.1, 0.1, 0.1, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 0.75,  0.05, 0.05, 0.05, 1.0);
	...
	pat = cairo_pattern_create_linear (0, 0, 0, knob_y);
	cairo_pattern_add_color_stop_rgba (pat, 1,  0.0, 0.0, 0.05, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 0.5,  0.0, 0.15, 0.2, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 0.,  0.0, 0.2, 0.25, 1.0);
...
static void bypass_expose(const gx_eternityUI * const ui,  const gx_controller * const switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(const gx_eternityUI * const ui,  const gx_controller * const controller) {
	if (controller->type < BSWITCH) knob_expose(ui, controller);
	else bypass_expose(ui, controller);
}

// general XWindow expose callback, 
static void _expose(const gx_eternityUI * const ui) {
...
static void controller_expose(const gx_eternityUI * const ui,  const gx_controller * const control) {
	cairo_push_group (ui->cr);
...
static void reset_request(gx_eternityUI * const ui) {
...
static void send_controller_event(gx_eternityUI * const ui, int controller) {
...
static void check_value_changed(gx_eternityUI * const ui, int i, const float * const value) {
...
static void check_is_active(gx_eternityUI * const ui, int i, bool set) {
...
static bool aligned(int x, int y, const gx_controller * const control, const gx_eternityUI * const ui) {
...
static bool get_active_ctl_num(gx_eternityUI * const ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num( const gx_eternityUI * const ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_eternityUI * const ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_eternityUI * const ui, int i) {
	float value;
...
static void switch_event(gx_eternityUI * const ui, int i) {
	float value = ui->controls[i].adj.value ? 0.0 : 1.0;
	check_value_changed(ui, i, &value);
}

// left mouse button is pressed, generate a switch event, or set controller active
static void button1_event(gx_eternityUI * const ui, double* start_value) {
	int num;
...
static void motion_event(gx_eternityUI * const ui, double start_value, int m_y) {
...
static void set_key_value(gx_eternityUI * const ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_eternityUI * const ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_eternityUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_eternityUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_eternityUI * const ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void event_handler(gx_eternityUI * const ui) {
	XEvent xev;
